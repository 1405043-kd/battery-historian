...
func HTTPAnalyzeHandler(w http.ResponseWriter, r *http.Request) {
	...
	//commentKanak
	AnalyzeAndResponse(fs, counterGlobal, "not_possible")
...
func AnalyzeAndResponse( files map[string]UploadedFile,counterGlobal int64 , fileNameCSV string) {
	pd := &ParsedData{}
	defer pd.Cleanup()
	if err := pd.AnalyzeFiles(files,counterGlobal, fileNameCSV); err != nil {
...
func (pd *ParsedData) AnalyzeFiles(files map[string]UploadedFile, counterGlobal int64, fileNameCSV string) error {
	...
	if err := pd.parseBugReport(fB.FileName, string(fB.Contents), fB2.FileName, string(fB2.Contents), counterGlobal, fileNameCSV); err != nil {
...
func (pd *ParsedData) parseBugReport(fnameA, contentsA, fnameB, contentsB string, counterGlobal int64, fileNameCSV string) error {
	...
	doParsing = func(brDA, brDB *brData) {
		...
		file, err :=os.Create("gen1"+fileNameCSV+".csv")
		//
		...
		//file, err := os.Create("hoyasa.csv")
		log.Printf("Cannot create file", err)
		defer file.Close()
		...
		file1, err :=os.Create("gen2"+fileNameCSV+".csv")
		...
		file2, err :=os.Create("gen3"+fileNameCSV+".csv")
		...
		file3, err :=os.Create("gen4"+fileNameCSV+".csv")
		...
		file4, err :=os.Create("gen5"+fileNameCSV+".csv")
		...
		file5, err :=os.Create("gen6"+fileNameCSV+".csv")
		...
		file6, err :=os.Create("D:/csvFiles/"+"gen7"+fileNameCSV+".csv")
...
////new bugReportGenerator




func (pd *ParsedData) parseBugReportMOd(fnameA, contentsA, fnameB, contentsB string, counterGlobal int64) error {
	...
	doParsing = func(brDA, brDB *brData) {
		...
		file, err :=os.Create(strconv.FormatInt(counterGlobal,16)+"hoyasa.csv")
		//
		//if file_is_exists("hoyasa.csv")==true{
		//	file, err= os.Create("ahoyasa.csv")
		//} else {
		...
		//file1, err := os.Create("hoyasa1.csv")
		file1, err :=os.Create(strconv.FormatInt(counterGlobal,16)+"hoyasa1.csv")
		...
		//file2, err := os.Create("hoyasa2.csv")
		file2, err :=os.Create(strconv.FormatInt(counterGlobal,16)+"hoyasa2.csv")
		...
		//file3, err := os.Create("hoyasa3.csv")
		file3, err :=os.Create(strconv.FormatInt(counterGlobal,16)+"hoyasa3.csv")
		...
		//file4, err := os.Create("hoyasa4.csv")
		file4, err :=os.Create(strconv.FormatInt(counterGlobal,16)+"hoyasa4.csv")
		...
		//file5, err := os.Create("hoyasa5.csv")
		file5, err :=os.Create(strconv.FormatInt(counterGlobal,16)+"hoyasa5.csv")
		log.Printf("Cannot create file5", err)
		defer file5.Close()
		writer5 := csv.NewWriter(file5)
		defer writer5.Flush()
		...
		//file6, err := os.Create("hoyasa6.csv")
		file6, err :=os.Create(strconv.FormatInt(counterGlobal,16)+"hoyasa6.csv")
